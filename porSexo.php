<?php
	...
	$porSexo = "SELECT `Sexo`,count(*) as total from tablita group by `Sexo`";
	$resultado = $conexion->query($porSexo);
...
<head>
	...
	<title>Vista previa de accidentes por Sexo</title>
...
<body>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Sexo' ${item[`Sexo`]}`);
	...
	<label>MASCULINO <input type="checkbox" value="MASCULINO" checked></label> 
	<label>FEMENINO <input type="checkbox" value="FEMENINO" checked></label> 
	...
	<script>
		...
		function actualizargrafiquito(){
			...
			dataJavaScript.forEach(item=>{
				if(diaSeleccionados.includes(item['Sexo'])) {
					nuevasLineas.push(item['Sexo']);
