<?php
	...
	$porDireccionnormalizada = "SELECT `Dirección normalizada`,count(*) as total from tablita group by `Dirección normalizada`";
	$resultado = $conexion->query($porDireccionnormalizada);
...
<head>
	...
	<title>Vista previa de accidentes por Dirección normalizada</title>
...
<body>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Dirección normalizada' ${item[`Dirección normalizada`]}`);
		...
		data: {
			labels: lineas,
			datasets: [{
				label: 'Conteo por Dirección normalizada',
				data: contar,
