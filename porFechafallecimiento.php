<?php
	...
	$porFechafallecimiento = "SELECT `Fecha fallecimiento`,count(*) as total from tablita group by `Fecha fallecimiento`";

	$resultado = $conexion->query($porFechafallecimiento);
...
<head>
	...
	<title>Vista previa de accidentes por Fecha fallecimiento</title>
...
<body>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Fecha fallecimiento' ${item[`Fecha fallecimiento`]}`);
		...
		data: {
			labels: lineas,
			datasets: [{
				label: 'Conteo por Fecha fallecimiento',
				data: contar,
