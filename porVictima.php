<?php
	...
	$porVictima = "SELECT Victima, COUNT(*) AS total FROM tablita    GROUP BY Victima";

	$resultado = $conexion->query($porVictima);
...
<body>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Victima' ${item[`Victima`]}`);
...
<label>AUTO <input type="checkbox" value="AUTO" checked></label>
    ...
    <label>PEATON_MOTO <input type="checkbox" value="PEATON_MOTO" checked></label>
    ...
    <script>
		...
		function actualizargrafiquito(){
			...
			const VictimaSeleccionados = Array.from(checkboxs)
			...
			dataJavaScript.forEach(item=>{
				if(VictimaSeleccionados.includes(item['Victima'])) {
					nuevasLineas.push(item['Victima']);
