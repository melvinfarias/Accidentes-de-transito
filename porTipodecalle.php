<?php
	...
	$porTipodecalle = "SELECT `Tipo de calle`,count(*) as total from tablita group by `Tipo de calle`";

	$resultado = $conexion->query($porTipodecalle);
...
<head>
	...
	<title>Vista previa de accidentes por Tipo de calle</title>
...
<body>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Tipo de calle' ${item[`Tipo de calle`]}`);
		...
		data: {
			labels: lineas,
			datasets: [{
				label: 'Conteo por Tipo de calle',
				data: contar,
