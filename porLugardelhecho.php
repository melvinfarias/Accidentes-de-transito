<?php
	...
	$porLugardelhecho = "SELECT `Lugar del hecho`,count(*) as total from tablita group by `Lugar del hecho`";

	$resultado = $conexion->query($porLugardelhecho);
...
<head>
	...
	<title>Vista previa de accidentes por Lugar del hecho</title>
...
<body>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Lugar del hecho' ${item[`Lugar del hecho`]}`);
		...
		data: {
			labels: lineas,
			datasets: [{
				label: 'Conteo por Lugar del hecho',
				data: contar,
