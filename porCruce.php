<?php
	...
	$porCruce = "SELECT `Cruce`,count(*) as total from tablita group by `Cruce`";
	
	$resultado = $conexion->query($porCruce);

	$data=[];

	while($columna = $resultado->fetch_assoc() ){
		//echo"cruce: ". $columna["Cruce"] ." cantidad de accidentes: ". $columna["total"]."<br>";
		$data[]=$columna;
	}
...
<head>
	...
	<title>Vista previa de accidentes por cruce</title>
...
<header>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Cruce' ${item[`Cruce`]}`);
