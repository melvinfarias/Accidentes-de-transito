<?php
	...
	$porCalle = "SELECT `Calle`,count(*) as total from tablita group by `Calle`";
	$resultado = $conexion->query($porCalle);

	$data=[];

	while($columna = $resultado->fetch_assoc() ){
		//echo"Calle: ". $columna["Calle"] ." cantidad de accidentes: ". $columna["total"]."<br>";
		$data[]=$columna;
	}
...
<head>
	...
	<title>Vista previa de accidentes por Calle</title>
...
<body>
	...
	<script> 
		...
		const lineas = dataJavaScript.map(item => `'Calle' ${item[`Calle`]}`);
